...
void tabuSearch(const UndirectedGraph& graph, std::vector<unsigned>& coloring, unsigned maxIter, size_t tabuListSize) {
    ...
    for (unsigned iter = 0; iter < maxIter; ++iter) {
        std::vector<std::pair<size_t, unsigned>> neighbours = neigh(graph, current_solution);
