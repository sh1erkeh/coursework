...
void printEvents(const QVector<Event>& events,
                 const std::vector<unsigned>& coloring) {
    std::map<int, QVector<std::pair<unsigned, QString>>> groupedData;
    for (int i = 0; i < events.size(); ++i) {
        groupedData[events[i].group_name].append(
            {coloring[i], events[i].subject_name});
    }

    for (const auto& group : groupedData) {
        QString fileName =
            "Группа" +
            QString::fromStdString(std::to_string(group.first + 1) + ".csv");
        QFile file(fileName);
        ...
        for (const auto& pair : group.second) {
            // std::cout << (pair.first - 1) / 7 << ' ' << (pair.first - 1) % 7
            // << ' ' << pair.first - 1 << std::endl;
            timetable[(pair.first - 1) / 6][(pair.first - 1) % 6 + 1] =
                pair.second;
        }
