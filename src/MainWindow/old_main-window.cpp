...
MainWindow::MainWindow(const QString& login, QWidget* parent)
    ...
    QVBoxLayout* scrollLayout =
        qobject_cast<QVBoxLayout*>(scrollWidget->layout());
    if (!scrollLayout) {
    ...
    QVBoxLayout* scrollLayout_4 =
        qobject_cast<QVBoxLayout*>(scrollWidget_4->layout());
    if (!scrollLayout_4) {
...
void MainWindow::on_saveButton_clicked() {
    ...
    query.clear();
    query.prepare(
        "INSERT INTO GROUPS (name, subjects, quantities) VALUES (:name, "
        ":subjects, :quantities)");
    ...
    for (int i = 0; i < nameCount; ++i) {
        ...
        QList<QPlainTextEdit*> textEdits =
            name->findChildren<QPlainTextEdit*>();
        ...
        for (int j = 0; j < textEdits.size(); ++j) {
            ...
            if (textEdit && textSpin) {
                ...
                if (!getIdQuery.exec()) {
                    qDebug() << "Ошибка поиска предмета: "
                             << getIdQuery.lastError().text();
                    continue;
                }
                ...
                if (!getIdQuery.next()) {
                    ...
                    if (!query1.exec()) {
                        qDebug() << "Ошибка сохранения предмета: "
        ...
        if (!subjectList.isEmpty() && !quantitiesList.isEmpty()) {
            ...
            if (!query.exec()) {
                qDebug() << "Ошибка сохранения GROUPS: "
                         << query.lastError().text();
            }
        } else {
            qDebug() << "Пропущена страница " << i
                     << ", так как нет данных для сохранения.";
        }
    }
}
...
void MainWindow::addNewWidget(QVBoxLayout* scrollLayout,
    ...
    if (checking_label[pushButton_6] == 0 &&
        ...
        QSpacerItem* gap = new QSpacerItem(10, 10, QSizePolicy::Expanding,
                                           QSizePolicy::Minimum);
        QSpacerItem* gap1 =
            new QSpacerItem(10, 10, QSizePolicy::Minimum, QSizePolicy::Minimum);
...
void MainWindow::onSwitch4Clicked(const QString& buttonText) {
    ...
    name_scroll_area->setWidgetResizable(true);
    name_scroll_area->setSizePolicy(QSizePolicy::Minimum,
    ...
    QGridLayout* name_vertical_layout =
        qobject_cast<QGridLayout*>(name->layout());
    ...
    QVBoxLayout* name_layout =
        qobject_cast<QVBoxLayout*>(name_scrollWidget->layout());
    if (!name_layout) {
    ...
    name_scroll_area->verticalScrollBar()->setValue(
        name_scroll_area->verticalScrollBar()->maximum());
    ui->stackedWidget->addWidget(name);
    ...
    connect(name_button_add, &QPushButton::clicked, this,
            [this, name_layout, name_scroll_area, name_button_add]() {
                addNewWidget(name_layout, name_scroll_area, name_button_add);
            });

    connect(name_button_save, &QPushButton::clicked, this,
            &MainWindow::on_saveButton_clicked);
}
...
void MainWindow::onButton12Clicked() {
    ...
    clrAlgo::tabuSearch<ObjectiveFunction, GetNeighbours>(graph, coloring, 1000,
                                                          10);
