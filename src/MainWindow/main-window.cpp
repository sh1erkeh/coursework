...
MainWindow::MainWindow(const QString& login, QWidget* parent)
    ...
    if (!db.open()){
        throw std::runtime_error("Error creating DB: " + db.lastError().text().toStdString());
    }
...
void MainWindow::on_saveButton_clicked() {
    ...
    if (!query.exec("DELETE FROM GROUPS")) {
        throw std::runtime_error("Could not clear database: " + query.lastError().text().toStdString());
    } else {
    ...
    for (int i = 0; i + 1 < pageCount; ++i) {
        ...
        for (int j = 1; j < textEdits.size(); ++j) {
            ...
            if (textEdit && textSpin) {
                ...
                if (content.isEmpty()) {
                    qDebug() << "Subject number " << j << " is empty. Skipping it";
                    continue;
                }
                ...
                if (!getIdQuery.exec()) {
                    throw std::runtime_error("Error finding subject: " + getIdQuery.lastError().text().toStdString());
                }

                if (!getIdQuery.next()) {
                    query1.bindValue(":name", content);
                    if (!query1.exec()) {
                        throw std::runtime_error("Error saving subject: " + query1.lastError().text().toStdString());
                    }
                }
        ...
        if (!subjectList.isEmpty() && !amountList.isEmpty()) {
            ...
            if (!query.exec()) {
                throw std::runtime_error("Error saving GROUPS: " + query.lastError().text().toStdString());
            } else {
